...
Appendix A spells out exactly what `qif &oracle(&x,&y,&z) { phase(𝜋) }` desugars to, and Section 3.1 explains the key step: "Calling [phase()] within the qif causes the Z gate to be applied to the qubit owned by tmp. 
